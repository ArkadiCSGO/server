...
function openInterface()
  ...
  if not interface then -- hidden
	...
	compass.intercardinal.tickShow = false
...
local appid = '496079276035145728' -- Make an application @ https://discordapp.com/developers/applications/ ID can be found there.
local asset = 'sans_titre-1' -- Go to https://discordapp.com/developers/applications/APPID/rich-presence/assets
...
function SetRP()
    ...
    SetRichPresence(#players .. " StarsBlitziens [" .. id .."]")--name)-- .. ' playing on [server name]')
...
function OpenPersonnelMenu()
	...
	ESX.TriggerServerCallback('NB:getUsergroup', function(group)
		...
		table.insert(elements, {label = 'Vestiaire', value = 'player_dressing'})
		table.insert(elements, {label = 'Carte d\'identité', value = 'id'})
		table.insert(elements, {label = 'Montrer sa Carte d\'identité', value = 'id1'})
		table.insert(elements, {label = 'Enlever/Mettre Poste Radio', value = 'radio'})
		table.insert(elements, {label = 'Cacher/Mettre le HUD', value = 'openInterface'})
		table.insert(elements, {label = 'Enlever/Mettre Cinematique', value = 'movie'})
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if playergroup == 'mod' then
					table.insert(elements, {label = 'Annonce',    				value = 'annonce'})
				...
				if playergroup == 'superadmin' or playergroup == 'owner' then
					table.insert(elements, {label = 'Test Purge',    				value = 'purge'})
					table.insert(elements, {label = 'Annonce',    				value = 'annonce'})
					table.insert(elements, {label = 'BlackOut',    				value = 'blackout'})
					table.insert(elements, {label = 'Enlever BlackOut',    				value = 'blackoutf'})
					table.insert(elements, {label = 'TP sur joueur',    				value = 'menuperso_modo_tp_toplayer'})
					table.insert(elements, {label = 'TP joueur sur moi',             	value = 'menuperso_modo_tp_playertome'})
					table.insert(elements, {label = 'TP sur coordonées [WIP]',			value = 'menuperso_modo_tp_pos'})
					table.insert(elements, {label = 'NoClip',							value = 'menuperso_modo_no_clip'})
				...
				if data.current.value == 'menuperso_modo' then
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							...
							if data2.current.value == 'annonce' then
								annonce()
							end
							if data2.current.value == 'purge' then
								purge()
								print("Test de Purge")
							end
							if data2.current.value == 'blackout' then
								SetBlackout(true)
							end
							if data2.current.value == 'blackoutf' then
								SetBlackout(false)
							end
				...
				if data.current.value == 'openInterface' then
					openInterface()
				end
				
				if data.current.value == 'movie' then
					movie()
				end
				...
				if data.current.value == 'id1' then
					local player, distance = ESX.Game.GetClosestPlayer()

					if distance ~= -1 and distance <= 3.0 then
					  TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
						ESX.ShowNotification('Vous avez donner votre Carte d\'identité')
					else
				...
				if data.current.value == 'menuperso_gpsrapide' then
					ESX.UI.Menu.Open(
						...
						css		 = 'gps',
							...
							align    = 'top-left',
							elements = {
								{label = 'Auto-Ecole',     value = 'menuperso_gpsrapide_autoecole'},
						...
						function(data2, menu2)
							...
							if data2.current.value == 'menuperso_gpsrapide_autoecole' then
								x, y, z = Config.autoecole.x, Config.autoecole.y, Config.autoecole.z
...
ESX.UI.Menu.CloseAll()
	...
	local elements = {}
	table.insert(elements, {label = 'Désactiver', value = 'false'})
	table.insert(elements, {label = 'Limiteur a 30 Km/h', value = '30'})
	table.insert(elements, {label = 'Limiteur a 50 Km/h', value = '50'})
	table.insert(elements, {label = 'Limiteur a 80 Km/h', value = '80'})
	table.insert(elements, {label = 'Limiteur a 100 Km/h', value = '100'})
	table.insert(elements, {label = 'Limiteur a 130 Km/h', value = '130'})
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
--------------------- GESTION MOTEUR
			if data.current.value == 'false' then
				speed = 0/3.6
				local ped = GetPlayerPed(-1)
			...
			if data.current.value == '100' then
				speed = 100/3.6
				local ped = GetPlayerPed(-1)
				local vehicle = GetVehiclePedIsIn(ped, false)
...
function OpenVehiculeMenu()
	...
	table.insert(elements, {label = 'Limitateur',			value = 'limitateur'})
	if vehiculeON then
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.value == 'limitateur' then
				Limitateur()
			end
--------------------- GESTION MOTEUR
...
Citizen.CreateThread(function()
	while true do
		...
		if inputann == 2 then
			...
			TriggerServerEvent('SyncAnnonce', inputText)
...
RegisterNetEvent('DisplayAnnonce')
AddEventHandler('DisplayAnnonce',function(inputText)
...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("Weazel News")
		PushScaleformMovieFunctionParameterString(inputText)
...
Citizen.CreateThread(function()
	while true do
		...
		if inputpurge == 2 then
			local inputText = GetOnscreenKeyboardResult()
			TriggerServerEvent('SyncPurge', inputText)
...
RegisterNetEvent('DisplayPurge')
AddEventHandler('DisplayPurge',function(inputText)
TriggerServerEvent('InteractSound_SV:PlayOnAll', 'purge', 0.1)
Citizen.CreateThread(function()
    function Initialize(scaleform)
        local scaleform = RequestScaleformMovie(scaleform)
...
RegisterNetEvent('NB:openMenuVehicle')
AddEventHandler('NB:openMenuVehicle', function()
	Vehicule()
end)
